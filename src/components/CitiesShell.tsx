...
const CitiesShell: FC<CitiesProps> = ({cities}: CitiesProps) => {
    ...
    let r = ' ту';
    let v = ['р', 'ра', 'ров'];
    let n = cities.tours.length
    
    let str1 = n + r + sklonenie(v, n);   
       
    
    let abc = cities.tours.map(tours => tours.time)
    const sumTimeTour = abc.reduce(function(acc, val) { return acc + val; }, 0)
    let tee = ''
    let ass = 0
    ...
    return (
            <Link to={`/cities/${cities.id}`}  className='__tour'>
                <div className='__img_tour'>
                    <div className='img_container'>
                        <img className='img' alt='Тур, ТурБокс, TourBox, дешовые онлайн туры по всей россии.' src={cities.img}/>
                        <h1>{cities.city}</h1>
                    </div>
                </div>
                    ...
                    <ul>
                        <div>
                            <li>
                                ...
                                <span className='_tr__on_t'>{ass} {tee}</span>
                            </li>
                            <li>
                        ...
                        <li className='info__t_last' style={{display:'grid', textAlign:'center'}}>{str1}</li>
                    </ul>
                </div>
            </Link>
    );
